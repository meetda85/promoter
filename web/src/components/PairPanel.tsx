...
export function PairPanel({ link }: { link: HostLinkInfo }) {
  ...
  const statusText: Record<string, string> = {
    ...
    open: link.remotes > 0 ? `${link.remotes} mando(s) conectado(s)` : 'Sala abierta, esperando mando',
    retrying: link.detail || 'Reintentando…',
    error: link.detail || 'Sin conexión con el servidor',
  }

  const tone =
    link.status === 'open' ? (link.remotes > 0 ? 'ok' : 'warn') : link.status === 'off' ? undefined : 'bad'
  ...
  return (
    <div>
      <div className="card" style={{ textAlign: 'center' }}>
        ...
        <div className="muted">
          Escanea el código con el otro teléfono, o abre <b>{httpBase(remote.serverUrl)}</b> y escribe
          este código en «Usar como mando».
        </div>
      ...
      <div className="card">
        ...
        <div className="muted">
          Si has abierto la app desde la dirección IP del ordenador que ejecuta el servidor, no hace
          falta tocar nada: los dos teléfonos se encuentran solos por la wifi. Rellena este campo
          sólo si quieres apuntar a otro servidor (por ejemplo uno público, para controlar el
          teleprompter desde fuera de la red local).
        </div>
